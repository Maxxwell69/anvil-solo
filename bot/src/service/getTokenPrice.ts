...
import { subBalance } from "../library";
...
const getTokenPrice = async (
  ...
  try {
    const apiUrl = `${ConfigService.getConfig().services.dexScreenerApiUrl}/${targetToken}`;
...
export const convertTokenAmount = async (
  ...
  try {
    ...
    if (price === null) {
      return await retryConvertTokenAmount(sourceAmount, sourceToken, targetToken);
    } else {
